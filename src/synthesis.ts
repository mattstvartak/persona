...
export interface CommunicationTraits {
  // Measured from user messages
  avgMessageLength: number;       // chars
  avgSentenceLength: number;      // words per sentence
  usesEmoji: boolean;
  usesSlang: boolean;
  formalityLevel: number;         // 0 (very casual) to 1 (very formal)
  technicalDepth: number;         // 0 (non-technical) to 1 (deep technical)
  humorFrequency: number;         // 0 (never) to 1 (frequent)
  directness: number;             // 0 (indirect/polite) to 1 (blunt/direct)
...
const DEFAULT_TRAITS: CommunicationTraits = {
  avgMessageLength: 0,
  avgSentenceLength: 0,
  usesEmoji: false,
  usesSlang: false,
 ...
 */
export function analyzeUserMessages(messages: string[]): CommunicationTraits {
  if (messages.length === 0) return { ...DEFAULT_TRAITS };

  const traits: CommunicationTraits = { ...DEFAULT_TRAITS, sampleSize: messages.length };
  ...
  const allSentences = messages.flatMap(m =>
    m.split(/[.!?]+/).filter(s => s.trim().length > 5)
  );
  ...
  let formalScore = 0.5;
  // Formal markers
  if (/\b(please|kindly|would you|could you|I would appreciate)\b/i.test(allText)) formalScore += 0.15;
  if (/\b(Dear|Regards|Best|Thank you for)\b/.test(allText)) formalScore += 0.2;
  // Informal markers
  if (traits.usesSlang) formalScore -= 0.15;
  if (traits.usesProfanity) formalScore -= 0.2;
  if (traits.usesEmoji) formalScore -= 0.1;
  if (/\b(hey|hi|yo|sup)\b/i.test(allText)) formalScore -= 0.1;
  ...
  let directScore = 0.5;
  // Direct markers
  if (/^(do|make|fix|add|remove|change|update|delete|run|show|give)/im.test(allText)) directScore += 0.15;
  if (traits.prefersTerse) directScore += 0.15;
  if (/\bjust\b/i.test(allText)) directScore += 0.1;
  // Indirect markers
  if (/\b(maybe|perhaps|possibly|I was wondering|would it be possible)\b/i.test(allText)) directScore -= 0.2;
  if (/\b(I think|in my opinion|it seems like)\b/i.test(allText)) directScore -= 0.1;
  traits.directness = Math.max(0, Math.min(1, directScore));

  // Question style
  const questions = messages.filter(m => m.includes('?'));
  if (questions.length > 0) {
    const exploratoryCount = questions.filter(q =>
      /\b(what if|how about|could we|would it|I wonder|explore|consider)\b/i.test(q)
    ).length;
    const directCount = questions.filter(q =>
      /^(how do|what is|where is|can you|does|is there|show me)\b/i.test(q.trim())
    ).length;

    if (exploratoryCount > directCount * 1.5) traits.questionStyle = 'exploratory';
    else if (directCount > exploratoryCount * 1.5) traits.questionStyle = 'direct';
    else traits.questionStyle = 'mixed';
  }
...
): { personality: string; style: string; skill: string } {
...
function buildPersonality(
  ...
  if (traits.sampleSize < 5) return ''; // Not enough data yet

  const lines: string[] = ['# Personality', '', IMMUTABLE_PRINCIPLES, ''];
...
function buildStyle(
  ...
  if (traits.sampleSize < 3) return ''; // Not enough data
...
function buildSkill(
  ...
  if (traits.sampleSize < 5) return '';

  const lines: string[] = ['# Working Style', ''];
 ...
 */
export function updateSoulFromSynthesis(
  config: PersonaConfig,
  messages: string[]
): { updated: boolean; traits: CommunicationTraits; changes: string[] } {
