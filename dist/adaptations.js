...
let currentSession = { ...DEFAULT_SESSION_STATE };
...
export function getAdaptations(config, category) {
    ...
    if (category) {
        const negAssoc = traitState.emotionalAssociations.find(a => category.toLowerCase().includes(a.topic.toLowerCase()) && a.valence < -0.3);
        if (negAssoc) {
 ...
 */
export function getProfileSummary(config) {
    const profile = loadProfile(config);
    const traitState = loadTraitState(config);
    const lines = [];
    ...
    if (prefs.prefersCodeFirst)
        lines.push('Prefers: code first');
    if (prefs.prefersBulletPoints)
        lines.push('Prefers: bullet points');
    if (prefs.avoidPatterns.length > 0)
        lines.push(`Avoids: ${prefs.avoidPatterns.join(', ')}`);
    if (prefs.preferredPatterns.length > 0)
        lines.push(`Likes: ${prefs.preferredPatterns.join(', ')}`);
    // Big Five summary
    if (traitState.bigFive.reliable) {
        const bf = traitState.bigFive;
