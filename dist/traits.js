 ...
 */
function inferTraitSignals(message) {
    ...
    const conscientiousness = clamp(Math.min(1, structureMarkers / 3) * 0.4 +
        Math.min(1, specifics / 4) * 0.3 +
        (wordCount > 50 ? 0.3 : wordCount > 20 ? 0.15 : 0), 0, 1);
    // ── Extraversion ──────────────────────────────────────────
    ...
    const extraversion = clamp(Math.min(1, exclamations / 3) * 0.25 +
        Math.min(1, socialRefs / 3) * 0.25 +
        Math.min(1, personalAnecdotes / 2) * 0.25 +
        Math.min(1, wordCount / 200) * 0.25, 0, 1);
    // ── Agreeableness ─────────────────────────────────────────
    ...
    const agreeableness = clamp((Math.min(1, hedges / 4) * 0.35 + Math.min(1, gratitude / 3) * 0.35) -
    ...
    const neuroticism = clamp(Math.min(1, negativeWords / 4) * 0.4 +
        Math.min(1, catastrophizing / 3) * 0.35 +
        Math.min(1, reassurance / 3) * 0.25, 0, 1);
 ...
 */
export function updateBigFive(current, message) {
