...
export interface ConsolidationResult {
  ...
  traitUpdates: string[];
  emotionalDecay: number;  // associations decayed
 ...
 */
export function runConsolidation(config: PersonaConfig): ConsolidationResult {
  ...
  traitState.emotionalAssociations = traitState.emotionalAssociations.filter(
    a => Math.abs(a.valence) > 0.05 || a.exposureCount > 5
  );
  ...
  if (history.length >= 5) {
    ...
    if (older5.length >= 3) {
      ...
      const dims: (keyof typeof recentAvg)[] = ['formality', 'energy', 'verbosity', 'humor', 'specificity'];
  ...
  if ((counts.approval ?? 0) > 5 && (counts.correction ?? 0) > 5) {
    ...
    if (ratio > 0.5 && ratio < 2) {
      result.contradictions.push(
        'Mixed signals: approval and correction rates are both high. ' +
  ...
  if (profile.stats.approvalRate > 0.8 && profile.stats.totalSignals > 30) {
    result.contradictions.push(
      'Warning: approval rate is above 80%. Check for sycophancy drift. ' +
...
function averageStyle(snapshots: SessionSummary['styleSnapshot'][]): SessionSummary['styleSnapshot'] {
