...
async function main() {
    ...
    try {
        ...
        recordSignal(config, 'praise', 'User reacted strongly positive ("Honestly I love knucklebones") to the Knucklebones title suggestion.', undefined, 'game-design');
        recordSignal(config, 'correction', '"Don\'t resize my browser windows please" — while testing responsive breakpoints via claude-in-chrome I called resize_window twice; Matt does not want his browser windows resized. Use iframe-viewport injection or other techniques instead.', undefined, 'code');
        for (let i = 0; i < 3; i++) {
        ...
        assert(avoids.some(p => p.includes('resize my browser windows')), `avoid pattern should capture the quoted directive, got ${JSON.stringify(avoids)}`);
        assert(likes.some(p => p.includes('knucklebones')), `like pattern should capture the quoted preference, got ${JSON.stringify(likes)}`);
        // ── Idempotency: rebuild N more times, nothing may drift ───────
        for (let i = 0; i < 5; i++)
            rebuildProfile(config, signals);
        const after = loadProfile(config);
        assert(after.topicPreferences.code.signalCount === first.topicPreferences.code.signalCount, `signalCount inflated across rebuilds: ${first.topicPreferences.code.signalCount} -> ${after.topicPreferences.code.signalCount}`);
        assert(after.topicPreferences.code.satisfaction === first.topicPreferences.code.satisfaction, `satisfaction drifted across rebuilds: ${first.topicPreferences.code.satisfaction} -> ${after.topicPreferences.code.satisfaction}`);
        assert(after.stylePreferences.verbosity === first.stylePreferences.verbosity, `verbosity drifted across rebuilds: ${first.stylePreferences.verbosity} -> ${after.stylePreferences.verbosity}`);
        assert(after.stylePreferences.avoidPatterns.length === first.stylePreferences.avoidPatterns.length, 'avoidPatterns grew across rebuilds');
        // signalCount now reflects reality: 4 code-category signals.
        assert(after.topicPreferences.code.signalCount === 4, `code signalCount should equal actual signals (4), got ${after.topicPreferences.code.signalCount}`);
...
//# sourceMappingURL=profile-rebuild.js.map