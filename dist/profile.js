...
export function rebuildProfile(config, signals) {
    ...
    const ordered = [...recent].sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
...
function extractionScopes(signal) {
...
function pushPattern(list, pattern) {
    if (pattern.length < 5)
        return;
