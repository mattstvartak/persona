...
function extractStylePatterns(signal: BehavioralSignal, prefs: StylePreferences): void {
  ...
  if (avoidMatches) {
    ...
    if (!prefs.avoidPatterns.includes(pattern)) {
      ...
      if (prefs.avoidPatterns.length > 20) prefs.avoidPatterns = prefs.avoidPatterns.slice(-20);
    }
  }
...
function extractPreferredPatterns(signal: BehavioralSignal, prefs: StylePreferences): void {
  ...
  if (likeMatches) {
    ...
    if (!prefs.preferredPatterns.includes(pattern)) {
      ...
      if (prefs.preferredPatterns.length > 20) prefs.preferredPatterns = prefs.preferredPatterns.slice(-20);
    }
  }
}
